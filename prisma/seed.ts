...
const LessonKind = { TEXT: 'TEXT', VIDEO: 'VIDEO', DOCUMENT: 'DOCUMENT', QUIZ: 'QUIZ' } as const;
type LessonKindValue = (typeof LessonKind)[keyof typeof LessonKind];
...
async function main() {
  ...
  await prisma.membership.upsert({
    ...
    update: { role: 'OWNER', status: 'ACTIVE', jobTitle: 'Platform Administrator' },
    create: { userId: admin.id, organizationId: organization.id, role: 'OWNER', status: 'ACTIVE', jobTitle: 'Platform Administrator' },
  });

  await prisma.membership.upsert({
    where: { userId_organizationId: { userId: employee.id, organizationId: organization.id } },
    update: { role: 'MEMBER', status: 'ACTIVE', jobTitle: 'Field Specialist' },
    create: { userId: employee.id, organizationId: organization.id, role: 'MEMBER', status: 'ACTIVE', jobTitle: 'Field Specialist' },
  });

  const safety = await upsertCourse(organization.id, {
    ...
    { title: 'Safety Starts Before the Incident', kind: LessonKind.TEXT, content: 'A safe workplace depends on noticing risk before somebody gets hurt. Scan your work area before beginning a task. Look for blocked exits, damaged equipment, trip hazards, exposed wiring, spills, missing guards, unstable loads, and any condition that differs from normal. If a hazard can be corrected safely, correct it. If it cannot, isolate the area and notify the appropriate supervisor. Never assume another person has already reported it.' },
    { title: 'Recognizing and Controlling Hazards', kind: LessonKind.TEXT, content: 'Use the hierarchy of controls when deciding how to reduce risk. Removing the hazard is stronger than relying only on personal protective equipment. When elimination is not possible, use engineering controls, administrative procedures, training, and PPE as appropriate. Stop work when conditions create an immediate danger and escalate the issue instead of improvising around it.' },
    { title: 'Emergency Response', kind: LessonKind.VIDEO, content: 'During an emergency, protect life first. Follow posted evacuation or shelter procedures, call emergency services when needed, and use designated assembly areas. Do not re-enter an evacuated area until authorized. This demo lesson supports an external training-video URL so a company can attach its own media.', mediaUrl: 'https://www.osha.gov/emergency-preparedness' },
    { title: 'How to Report an Incident', kind: LessonKind.DOCUMENT, content: 'Report injuries, near misses, property damage, unsafe conditions, and security events as soon as possible. Record what happened, where and when it happened, who was involved, immediate actions taken, and any witnesses. Describe facts rather than assigning blame. Prompt reporting helps the organization protect employees, preserve evidence, identify patterns, and correct hazards.', mediaUrl: 'https://www.osha.gov/recordkeeping' },
    { title: 'Safety Knowledge Check', kind: LessonKind.QUIZ, content: 'Score at least 80% to complete this course and receive a one-year training credential.', questions: [
      { prompt: 'What should you do first when you notice a serious hazard that cannot be corrected safely?', options: ['Ignore it if nobody is nearby', 'Isolate the area and notify the appropriate person', 'Wait until the end of the shift', 'Take a photo and continue working'], correctAnswer: 'Isolate the area and notify the appropriate person', explanation: 'Unsafe conditions should be controlled and escalated rather than worked around.' },
      { prompt: 'Which control is generally stronger than relying only on PPE?', options: ['Eliminating the hazard', 'Working faster', 'Posting a reminder', 'Signing a waiver'], correctAnswer: 'Eliminating the hazard', explanation: 'Removing a hazard reduces exposure at the source.' },
      { prompt: 'Which event should be reported?', options: ['Only injuries requiring hospitalization', 'Only property damage', 'Injuries, near misses, hazards, and security events', 'Nothing unless a manager witnessed it'], correctAnswer: 'Injuries, near misses, hazards, and security events', explanation: 'Near misses and unsafe conditions can reveal problems before a more serious event occurs.' },
      { prompt: 'Incident reports should primarily contain what?', options: ['Facts about what happened', 'Who deserves blame', 'Rumors from coworkers', 'Only the final cost'], correctAnswer: 'Facts about what happened', explanation: 'Factual reporting supports a useful investigation and corrective action.' },
      { prompt: 'After an evacuation, when should you re-enter the area?', options: ['When you think it is safe', 'After five minutes', 'Only when authorized', 'When a coworker returns'], correctAnswer: 'Only when authorized', explanation: 'Re-entry should happen only after the responsible authority gives clearance.' },
    ] },
  ]);

  const orientation = await upsertCourse(organization.id, {
  ...
  const cyber = await upsertCourse(organization.id, {
  ...
  for (const course of [safety, orientation, cyber]) {
    await prisma.enrollment.upsert({
      ...
      update: {},
      create: { organizationId: organization.id, userId: employee.id, courseId: course.id, dueAt: new Date(Date.now() + 14 * 24 * 60 * 60 * 1000) },
    });
  }

  await prisma.activity.create({ data: { organizationId: organization.id, actorId: admin.id, type: 'system', message: 'ICA Unified demo learning catalog synchronized.' } });
}
