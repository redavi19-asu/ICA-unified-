...
export async function POST(
  ...
  if (certificateRule === 'ATTENDANCE' || certificateRule === 'COMPLETE_EVENT') {
    ...
    if (existing) {
      ...
      credentialCode = `ICA-${randomUUID().split('-')[0].toUpperCase()}-${Date.now().toString(36).toUpperCase()}`;
  ...
  await prisma.activity.create({
    data: {
      ...
      message: `${membership.user.name} checked in to ${event.eventName}${credits > 0 ? ` and earned ${credits} ${category} credit(s)` : ''}.`,
    },
  });
  ...
  return NextResponse.json({
    ...
    credentialCode,
    message: credits > 0
      ? `Checked in. ${credits} ${category} credit(s) were added to your record.`
